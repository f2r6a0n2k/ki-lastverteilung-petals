# KI Worker Installation für Windows (PowerShell - als Administrator)
# Verwendung: .\install_worker_windows.ps1 [PORT] [MODELL]
# Beispiel: .\install_worker_windows.ps1 8080 bartowski/Llama-3.2-3B-Instruct-GGUF
...
$WORKER_NAME = "KI-Worker-Windows-$Port"

Write-Host "=== KI Worker Installation (Windows) ==="
...
# Firewall-Regel
New-NetFirewallRule -DisplayName "KI-Worker $Port" -Direction Inbound -LocalPort $Port -Protocol TCP -Action Allow -ErrorAction SilentlyContinue

# Start-Skript erstellen
$StartScript = "$env:USERPROFILE\start_ki_worker.ps1"
@"
# KI Worker Start-Skript
`$Port = $Port
`$Model = "$Model"
`$WORKER_NAME = "$WORKER_NAME"
Write-Host "Starte KI Worker: `$WORKER_NAME auf Port `$Port mit Modell: `$Model"
...
Write-Host "✅ KI Worker läuft auf Port $Port"
